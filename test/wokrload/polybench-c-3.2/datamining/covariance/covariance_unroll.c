...
static void init_array(int m, int n, DATA_TYPE* float_n,
    ...
    *float_n = 1.2;
    ...
    for (i = 0; i < M; i++)
        for (j = 0; j < N; j++) data[i][j] = ((DATA_TYPE)i * j) / M;
}
...
#pragma scop
    ...
    for (j1 = 0; j1 < _PB_M; j1++)
        for (j2 = j1; j2 < _PB_M; j2++) {
            ...
            symmat[j1][j2] = 0.0;
            for (i = 0; i < _PB_N; i++)
                symmat[j2][j1] += data[i][j1] + data[i][j2];
...
int main(int argc, char** argv) {
    ...
    /* Run kernel. */
    kernel_covariance(m, n, float_n, POLYBENCH_ARRAY(data),
                      POLYBENCH_ARRAY(symmat), POLYBENCH_ARRAY(mean));
