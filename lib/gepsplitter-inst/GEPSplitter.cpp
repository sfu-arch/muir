...
static RegisterPass<GEPSplitter> X("split-geps", "split complex GEPs into simple GEPs");
}  // namespace gepsplitter


bool
GEPSplitter::runOnFunction(Function& F) {
  bool Changed = false;
...
void
GEPSplitter::getAnalysisUsage(AnalysisUsage& AU) const {
  AU.setPreservesCFG();
}
