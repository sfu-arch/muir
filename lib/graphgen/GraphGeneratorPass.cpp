...
template <class T> Instruction *findParallelInstruction(Function &F) {
  for (auto &BB : F) {
    for (auto &I : BB) {
      if (isa<T>(I))
        return &I;
  ...
  for (auto B : _loop->blocks()) {
    for (auto &I : *B) {
      ...
      for (auto OI = I.op_begin(); OI != I.op_end(); OI++) {
        Value *_value = *OI;
        if (definedInCaller(SetVector<BasicBlock *>(_loop->blocks().begin(),
                                                    _loop->blocks().end()),
                            _value)) {
...
void UpdateLiveOutConnections(Loop *_loop, LoopNode *_loop_node,
  ...
  for (auto B : _loop->blocks()) {
    for (auto &I : *B) {
       ...
       */
      for (auto *U : I.users()) {
        if (!definedInRegion(SetVector<BasicBlock *>(_loop->blocks().begin(),
                                                     _loop->blocks().end()),
                             U)) {
...
void GraphGeneratorPass::visitGetElementPtrInst(llvm::GetElementPtrInst &I) {
  ...
  if (src_type->isStructTy()) {
    map_value_node[&I] =
        this->dependency_graph->insertGepNode(I, gep_pass_ctx.GepStruct[&I]);
...
void GraphGeneratorPass::findDataPort(Function &F) {
  ...
  for (auto ins_it = inst_begin(F); ins_it != inst_end(F); ++ins_it) {
    for (uint32_t c = 0; c < ins_it->getNumOperands(); ++c) {
      ...
      if (auto fn = dyn_cast<llvm::Function>(operand))
        continue;
      if (auto target_bb = dyn_cast<llvm::BasicBlock>(operand)) {
        ...
        auto _node_dest =
            this->map_value_node.find(operand); // it should be supernode
        assert(isa<SuperNode>(_node_dest->second) &&
               "Destination node should be super node!");

        auto _node_src =
            this->map_value_node.find(&*ins_it); // it should be Insnode
        ...
        if (auto const_value = dyn_cast<llvm::ConstantInt>(operand)) {
          ...
          this->map_value_node[ins_it->getParent()]->addControlOutputPort(
              _const_node);
    ...
    if (auto _bb = dyn_cast<SuperNode>(this->map_value_node[&BB])) {
      for (auto &I : BB) {
        ...
        if (auto _ins = dyn_cast<InstructionNode>(this->map_value_node[&I])) {
          ...
          if (auto detach = dyn_cast<ReattachNode>(_ins))
            continue;
...
void GraphGeneratorPass::fillLoopDependencies(llvm::LoopInfo &loop_info) {
  ...
  for (auto &L : getLoops(loop_info)) {
    ...
    if (L->getParentLoop())
      _parent = loop_value_node[L->getParentLoop()];
    ...
    auto _src_br_inst_it = *std::find_if(
        ...
        [&L](auto const _node_it) {
          return !L->contains(dyn_cast<BranchNode>(_node_it)->getInstruction());
        });
    ...
    if (auto _latch_br =
            dyn_cast<BranchNode>(map_value_node[&L->getLoopLatch()->back()])) {
    ...
    for (auto B : L->blocks()) {
      ...
      for (auto &I : *B) {
         ...
         */
        for (auto *U : I.users()) {
          if (!definedInRegion(SetVector<BasicBlock *>(L->blocks().begin(),
                                                       L->blocks().end()),
                               U)) {
            auto new_live_out = _loop_node->insertLiveOutArgument(U);
