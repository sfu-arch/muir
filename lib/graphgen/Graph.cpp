...
template <class T> Node *findParallelNode(Graph *_graph) {
...
template <class T> std::vector<T *> getNodeList(Graph *_graph) {
  ...
  case PrintType::Scala:
    ...
    for (auto &_s_node : super_node_list) {
      for (auto _enable_iterator = _s_node->inputControl_begin();
        ...
        auto _input_index = _s_node->returnControlInputPortIndex(_input_node);
        auto _output_index =
            _input_node->returnControlOutputPortIndex(_s_node.get());
    ...
    for (auto &_s_node : super_node_list) {
      for (auto &_const_iterator : _s_node->cfps()) {
        this->outCode
            ...
            << _s_node->printOutputEnable(
                   PrintType::Scala,
                   _s_node->returnControlOutputPortIndex(&*_const_iterator)
                       .getID())
            << "\n\n";
      }
      ...
      for (auto _ins_iterator = _s_node->ins_begin();
        ...
        auto ff = std::find_if(
            ...
            [&_s_node](auto &arg) -> bool { return _s_node.get() == &*arg; });
    ...
    for (auto _fd_node : fdiv_list) {
      this->outCode
          ...
          << _fd_node->printMemReadOutput(
                 PrintType::Scala,
                 _fd_node->returnMemoryReadOutputPortIndex(this->getFPUNode())
                     .getID())
          << "\n  "
          << _fd_node->printMemReadInput(
                 PrintType::Scala,
                 _fd_node->returnMemoryReadInputPortIndex(this->getFPUNode())
                     .getID())
          << " <> "
...
void Graph::printScalaFunctionHeader() {
  ...
  for (auto &_ins : this->inst_list) {
    if (auto _fc = dyn_cast<CallNode>(_ins.get())) {
      ...
      _command = "    val $call_in = Flipped(Decoupled(new Call(List(32))))\n";
  ...
  _final_command.append("    val MemResp = Flipped(Valid(new MemResp))\n"
  ...
  _final_command = "class $module_nameDF(implicit p: Parameters)"
...
InstructionNode *Graph::insertBinaryOperatorNode(BinaryOperator &I) {
  ...
  auto ff =
      std::find_if(inst_list.begin(), inst_list.end(), [&I](auto &arg) -> bool {
...
InstructionNode *Graph::insertLoadNode(LoadInst &I) {
  ...
  inst_list.push_back(std::make_unique<LoadNode>(
      NodeInfo(inst_list.size(), "ld_" + std::to_string(inst_list.size())), &I,
      this->getMemoryUnit(), _load_list.size()));
...
InstructionNode *Graph::insertStoreNode(StoreInst &I) {
  ...
  inst_list.push_back(std::make_unique<StoreNode>(
      NodeInfo(inst_list.size(), "st_" + std::to_string(inst_list.size())), &I,
      this->getMemoryUnit(), _store_list.size()));
...
ArgumentNode *Graph::insertFunctionArgument(Argument &AR) {
  ...
  auto ff =
      std::find_if(arg_list.begin(), arg_list.end(), [&AR](auto &arg) -> bool {
...
GlobalValueNode *Graph::insertFunctionGlobalValue(GlobalValue &G) {
  ...
  auto ff = std::find_if(
      glob_list.begin(), glob_list.end(),
      [&G](GlobalValueNode &gl) -> bool { return gl.getGlobalValue() == &G; });
  return &*ff;
}
  ...
  case PrintType::Scala:
    ...
    for (auto &_l_node : loop_nodes) {
      for (auto _enable_iterator = _l_node->inputControl_begin();
        ...
        auto _input_index = _l_node->returnControlInputPortIndex(_input_node);

        auto _output_index =
            _input_node->returnControlOutputPortIndex(_l_node.get());
    ...
    for (auto &_l_node : loop_nodes) {
      ...
      for (auto &_ending_ins : _l_node->endings()) {
        for (auto &_cn_dependencies : _ending_ins->output_control_range()) {
          ...
          auto _output_index =
              _ending_ins->returnControlOutputPortIndex(_cn_dependencies);
        ...
        for (auto &_data_in : _live_in->input_data_range()) {
          this->outCode << "  "
                        ...
                        << _data_in->printOutputData(
                               PrintType::Scala,
                               _data_in
                                   ->returnDataOutputPortIndex(_live_in.get())
                                   .getID())
                        << "\n\n";
        }
      }
    ...
    for (auto &_l_node : loop_nodes) {
      for (auto &_live_in : _l_node->live_ins()) {
        for (auto &_data_out : _live_in->output_data_range()) {
          if (isa<ArgumentNode>(_data_out))
            continue;
          this->outCode
              << "  "
              << _data_out->printInputData(
                     PrintType::Scala,
                     _data_out->returnDataInputPortIndex(_live_in.get())
                         .getID())
              << " <> "
              << _live_in->printOutputData(
                     PrintType::Scala,
                     _live_in->returnDataOutputPortIndex(_data_out).getID())
              << "\n\n";
        }
      }
    }
        ...
        for (auto &_data_out : _live_out->input_data_range()) {
          this->outCode
              ...
              << _live_out->printInputData(
                     PrintType::Scala,
                     _live_out->returnDataInputPortIndex(_data_out).getID())
              << " <> "
              << _data_out->printOutputData(
                     PrintType::Scala,
                     _data_out->returnDataOutputPortIndex(_live_out.get())
                         .getID())
              << "\n\n";
        }
      }
...
void Graph::doInitialization() {
  ...
  for (auto &_node : const_int_list) {
    for (auto &_child : _node->output_data_range()) {
      if (isa<ArgumentNode>(&*_child))
        continue;
      this->insertEdge(
          Edge::EdgeType::DataTypeEdge,
          std::make_pair(&*_node, _node->returnDataOutputPortIndex(&*_child)),
          std::make_pair(&*_child, _child->returnDataInputPortIndex(&*_node)));
    }
  }
  ...
  for (auto &_node : inst_list) {
    ...
    for (auto &_child : _ptr->output_data_range()) {
      ...
      this->insertEdge(
          ...
          std::make_pair(&*_child, _child->returnDataInputPortIndex(_ptr)));
    }
  }
  ...
  for (auto &_arg : this->getSplitCall()->live_ins()) {
    for (auto &_node : _arg->output_data_range()) {
      ...
      this->insertEdge(
          Edge::EdgeType::DataTypeEdge,
          std::make_pair(&*_arg, _arg->returnDataOutputPortIndex(&*_node)),
          std::make_pair(&*_node, _node->returnDataInputPortIndex(&*_arg)));
    }
  }
  ...
  for (auto &_node : inst_list) {
    if (auto _ld_node = dyn_cast<LoadNode>(&*_node)) {
      ...
      insertEdge(
          ...
          std::make_pair(_ld_node, _ld_node->returnMemoryReadOutputPortIndex(
                                       getMemoryUnit())),
          std::make_pair(
              getMemoryUnit(),
      ...
      insertEdge(
          ...
          std::make_pair(_ld_node, _ld_node->returnMemoryReadInputPortIndex(
                                       getMemoryUnit())));
    } else if (auto _st_node = dyn_cast<StoreNode>(&*_node)) {
      // Adding edges
      insertEdge(
          Edge::MemoryWriteTypeEdge,
          std::make_pair(_st_node, _st_node->returnMemoryWriteOutputPortIndex(
                                       getMemoryUnit())),
          std::make_pair(
              getMemoryUnit(),
              getMemoryUnit()->returnMemoryWriteInputPortIndex(_st_node)));

      insertEdge(
          Edge::MemoryWriteTypeEdge,
          std::make_pair(
              getMemoryUnit(),
              getMemoryUnit()->returnMemoryWriteOutputPortIndex(_st_node)),
          std::make_pair(_st_node, _st_node->returnMemoryWriteInputPortIndex(
                                       getMemoryUnit())));
      ...
      insertEdge(
          Edge::MemoryReadTypeEdge,
          std::make_pair(getStackAllocator(),
